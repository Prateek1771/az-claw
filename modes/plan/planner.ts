...
// nullable (not optional): Groq strict json_schema requires every key in `required`
...
export async function generatePlan(goal: string) {
  ...
  // Two phases: Groq rejects tools + JSON output in one call, so research
  // with tools first, then a tool-free structured call for the plan.
  const research = await generateText({
    model: getAgentModel(),
    tools,
    stopWhen: stepCountIs(20),
    system: PLAN_INSTRUCTIONS(config.codebasePath, hasWeb),
    prompt: `User goal:\n${goal}\n\nResearch the codebase with the tools, then reply with a concise summary of findings relevant to planning this goal.`,
  });
  ...
  return {goal , researchSummary:validated.researchSummary ?? undefined , steps}
}
